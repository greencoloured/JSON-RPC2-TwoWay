...
sub run {
	my $rpc = shift or die "no JSON::RPC2::TwoWay";
	...
	eval { $rpc->register('dierequest', sub { die 'die in request handler!' }); };
	ok(!$@, 'register die request');
	...
	@res = $con->handle('{"jsonrpc":"2.0","method":"dierequest","id":"foo","params":{"foo":"bar"}}');
	like($res[1],  qr/error: -32001 Method threw error: die in request handler! at /, 'method threw error');
	...
	$writebuf='';
	$con->call('foo', {}, sub { die "die in response handler!" });
	$writebuf = decode_json($writebuf);
	$callid = $writebuf->{id};
	@res = eval { $con->handle('{"jsonrpc":"2.0","id":"'.$callid.'","result":{"foo":"bar"}}') };
	if ($con->{legacy_mode}) {
		ok($@, 'error raised by handle');
		ok(!@res, 'no errors from handle');
	} else {
		ok(!$@, 'no error raised by handle');
		ok(@res, 'errors from handle');
	}
